...
| SDTEST-1213 | `sidebar::tests::activity_without_a_panel_contributes_no_panel_width` | SDUC-443 | Green | An activity with no contextual rows hides the panel while not collapsed; counting the panel there would offset every terminal past a column that is not on screen. The two reasons to hide must not add up. |
