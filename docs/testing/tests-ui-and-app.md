...
| SDTEST-1717 | `workspace::user_home::tests::user_information_uses_one_role_source_and_hides_empty_values` | SDUC-484 | Green | A non-empty normalized CM role bag wins even over contradictory capability flags, duplicates disappear, custom slugs become readable, legacy bag-less accounts get exactly one coherent fallback, and whitespace-only optional values produce no field row. Manually validated on X11 on 2026-08-25: the two human role badges agreed with the header, both absent timestamp rows vanished, and the header retained only the e-mail. |
