...
| SDTEST-1601 | `ai.rs::conversation_titles_mark_new_and_legacy_truncation_with_an_ellipsis` | SDUC-414 | Green | New 56-character conversation titles append a Unicode ellipsis when the first message continues; legacy stored titles are recognized against the retained first user message and repaired for display. The history renderer also gives its title/meta column a definite post-button width so GPUI shapes an ellipsis instead of hard-clipping the row. |
...
| SDTEST-1392 | Linux runtime smoke: hidden start → `Ctrl+Shift+Space` | SDUC-434, SDUC-435 | Green | The Dock opened at 480×1048 while the `initializing full Workspace` trace remained absent, proving the standalone controller path does not start Workspace views or pollers. Repeated 2026-08-11 on X11 with composite/window captures: the theme-radius top-left/bottom-left pixels reveal the desktop, both screen-edge right corners stay square, the 56 px rail starts at the top edge, and the sole 44 px header occupies only the conversation column. |
