...
| SDTEST-1694 | `onboarding_view::tests::sdtest_1694_the_run_follows_the_mode_and_the_modes_slide_follows_capability` | SDUC-477 | Green | One table pins all three runs plus the cross cases: the sequence follows the effective mode while the closing modes slide follows `allowed_modes`, so a super-admin in User mode gets the User run with the Dev-accented slide. Across every tier and landing mode the modes slide appears exactly once, only when switchable, and always last. |
| SDTEST-1695 | `onboarding_view::tests::sdtest_1695_every_slide_resolves_its_copy` | SDUC-477 | Green | Every slide resolves `title`/`intro`/`media_caption` and a title/body pair per bullet, plus the shared mode and shortcut keys. `rust_i18n` renders a missing key as the key itself, so a typo in `key()`/`bullets()` or a forgotten locale line would otherwise ship `onboarding.dev.tunnels.presets_body` to the user. Uses the ambient locale deliberately — `set_locale` is process-global and races the suite; SDTEST-1302 already pins fr/en parity. |
| SDTEST-1696 | `onboarding_view::tests::sdtest_1696_every_slide_asset_is_embedded_and_listed` | SDUC-477 | Green | Each slide's artwork appears in both `main.rs` tables (`include_bytes!` and `Assets::list`). Neither is compiler-checked, and an unregistered path fails the image load silently, leaving an empty hero zone. |
| SDTEST-1697 | `onboarding_view::tests::sdtest_1697_runs_cover_every_slide` | SDUC-477 | Green | The union of every reachable run accounts for all fifteen slides, so the test-local `ALL_STEPS` cannot drift from the enum and no slide becomes unreachable. |
| SDTEST-1698 | *to write* — the tour card keeps its footer reachable at every window size and UI scale | SDUC-477 | **Red / P2** | GPUI integration: the card caps at 90% of window height, the body scrolls, and Passer/Précédent/Terminer stay visible on the longest run (Dev: three modes plus four shortcut rows). Manually validated on X11 on 2026-08-21 at 1210×810 for all three roles after the clipped-footer fix; the automated harness is still missing. |
| SDTEST-1699 | `workspace::palette::tests::sdtest_1699_quit_is_never_the_preselected_first_entry` | SDUC-152 | Green | La palette présélectionne son premier résultat. « Quitter » y figurait en tête, si bien qu'ouvrir la palette et valider fermait l'application — sans recours clavier, puisque Échap n'était pas délivré non plus. Le test vérifie sur les quatre combinaisons de rôle que Quitter reste joignable mais jamais en première position. |
| SDTEST-1700 | `settings::tests::sdtest_1700_shortcut_failures_are_classified_never_dumped` | SDUC-449 | Green | Aucun échec d'enregistrement de raccourci global ne doit atteindre l'écran sous forme de `Debug` Rust. L'onglet Général affichait littéralement `X11 error X11Error { error_kind: Access, error_code:`, tronqué au milieu, dès qu'une autre application détenait déjà la combinaison — le cas courant, pas une anomalie. Le test pinne les trois classes, dont la forme exacte relevée sur cette machine. |
| SDTEST-1701 | `sidebar::tests::rail_lists_activities_not_destinations` | SDUC-152 | Green | La liste des activités qui gardent un panneau est close et vérifiée par égalité, non par exception nommée : un panneau ne se justifie que s'il montre ce que sa propre vue ne montre pas. Cinq activités répétaient la liste rendue juste à côté. |
| SDTEST-1702 | `workspace::render::tests::mode_transition_loader_fades_in_holds_then_fades_out` | SDUC-151 | Green | La courbe du voile est rapportée à la durée de la transition en cours, désormais variable — longue à la première entrée dans un mode, courte au retour. Une courbe figée ferait disparaître le voile avant la fin ou le laisserait après. |
...
| SDTEST-1704 | `agent_console_view::tests::sdtest_1704_agent_context_panel_uses_scale_aware_explicit_rows` | SDUC-475 | Green | The 720/960 logical-pixel breakpoints select narrow, compact, and wide rows at both 1× and 2× UI scale. Agent, target and permissions remain three divided cells inside one context frame; they cannot fall back to intrinsic wrapping, while the free-form model override lives in the composer popover. |
| SDTEST-1706 | `agent_console_view::tests::sdtest_1706_agent_activity_deduplicates_and_keeps_a_bounded_popover_tail` | SDUC-475 | Green | Eight consecutive identical Ready labels collapse to one; the header popover exposes only the twelve newest labels from a longer technical trace, preventing control chatter from entering or pushing the conversation. |
| SDTEST-1705 | `workspace::user_home::tests::sdtest_1705_welcome_height_breakpoint_tracks_ui_scale` | SDUC-441 | Green | The welcome screen centers its authentication task and pins the publisher at normal heights, then switches to one scrollable column below 560 logical pixels; the same boundary holds at 1× and 2× App Font Size. |
