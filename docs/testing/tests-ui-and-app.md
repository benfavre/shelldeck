...
| SDTEST-1824 | `main.rs::same_process_attention_notifications_activate_only_explicit_open_actions` | SDUC-493 | Green | Native notification handles wait in a retained worker until activation or dismissal; only the default/open actions enqueue the exact in-process attention tuple, while dismissal and unknown actions cannot navigate. Cold-launch deep links remain deliberately out of scope. |
