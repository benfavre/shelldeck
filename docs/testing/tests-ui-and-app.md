...
| SDTEST-1737 | `workspace::workspaces::tests_file::tests::{existing_folder_executor_streams_intermediate_progress,native_worktree_executor_uses_exact_typed_arguments_and_completes_all_phases,launcher_prefills_all_external_kinds_through_one_validated_model,failed_multi_step_catalog_mutation_rolls_back_before_persistence,catalog_change_before_completion_prevents_native_attach,late_completion_from_prior_retry_has_no_terminal_side_effect,completion_during_cancellation_has_no_terminal_side_effect}` | SDUC-488, SDUC-489, SDUC-491 | Yellow | The production local adapter is wired for exact existing folders and ShellDeck-owned Git worktrees; the fake Git seam proves fixed typed inputs and monotonic progress without running a subprocess in unit tests. Existing reducer/UI tests pin manual/external intake, cancellation, revision conflicts, retry fencing, attach ownership and transactional catalog rollback. Linux/macOS/Windows CI compile the portable path. SSH execution, forced-process cancellation evidence, and a live archive/resume journey remain unsupported release gaps. |
