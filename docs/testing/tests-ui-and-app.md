...
| SDTEST-1827 | `workspace/platform_attention.rs::whole_poll_failure_hides_every_board_and_mapping_replacement_recovers_cleanly` | SDUC-493 | Green | A whole Platform poll failure hides every retained attention projection. Removing/remapping the active exact context retires its board and resync state, and a new target can be installed without restart or reuse of the retired target. |
| SDTEST-1828 | `workspace/platform_attention.rs::activation_routes_each_destination_to_a_visible_surface_before_read_custody` | SDUC-493 | Green | Workspace and retained-pane destinations require the Workspaces surface, Fleet sessions require Fleet, and the shared durable read gate does not consume the exact tuple when the verified surface-open result is false. |
