...
| ID | Location | SDUC | Status | Notes |
|---|---|---|---|---|
| SDTEST-1736 | `workspace::workspaces::tests_file::tests::{keyed_gpui_workspace_entity_retention_preserves_hidden_terminal_state,retained_terminal_activation_keeps_the_complete_runtime_config}` and `terminal_view::tests::{canonical_workspace_cwd_reaches_tabs_splits_and_cli_created_terminals,native_workspace_snapshot_reapplies_split_focus_and_ratio}` | SDUC-490 | Yellow | Tests prove stable GPUI workspace/terminal entity identity across A→B→A, native terminal snapshot round-tripping, canonical default-cwd inheritance for later local terminals, and retained-terminal runtime configuration hydration. Editor/files/browser pane restoration and a live multi-workspace PTY archive/resume journey are not covered and remain unsupported. |
