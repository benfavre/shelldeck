...
| SDTEST-1414 | *to write* — User/Support home dashboards route to their operational tabs | SDUC-440 | **Red / P1** | GPUI integration: both modes start on Accueil; every Support counter clears stale constraints and opens the exact advertised queue; the action banner and priority-list column keep distinct headings; priority-ticket and recent-request rows open their real detail; User quick actions select the exact list/composer; sync acts on the current Manage account; onboarding omits Dev cards/media/shortcuts for non-Dev roles. |
