...
| SDTEST-1553 | `companion_desktop.rs::dynamic_physics_refresh_tracks_captured_windows_on_cadence` | SDUC-448, SDUC-449, SDUC-451 | Green | After initial discovery, dynamic falls refresh captured stable IDs at a bounded cadence, track moved tops, and remove closed windows before collision. |
...
| SDTEST-1572 | `companion_desktop.rs::dynamic_physics_refresh_switches_to_stable_id_updates_after_initial_scan` | SDUC-448, SDUC-449, SDUC-451 | Green | A fall performs one full visible-window discovery, then uses targeted stable-ID refreshes so moved and closed platforms stay current without repeating expensive full X11 scans every frame interval. |
| SDTEST-1573 | `adabraka_gpui::platform::linux::x11::client::companion_external_window_tests::x11_frame_extents_require_cardinal_32_and_bounded_exact_values` | SDUC-448, SDUC-449, SDUC-451 | Green | X11 outer-frame collision geometry accepts only exact CARDINAL/32 four-value `_NET_FRAME_EXTENTS` payloads inside a defensive bound and rejects malformed, wrong-format, wrong-type, or oversized properties. |
