...
| SDTEST-1744 | `portable-pty::cmdbuilder::tests::explicit_missing_cwd_is_forwarded_instead_of_falling_back_home` (`#[cfg(windows)]`, SDPATCH-117) | SDUC-022, SDUC-490 | Yellow | The Windows CI runner applies the lightweight dependency patch and compile-checks the patched crate's tests directly, proving the cfg-gated `current_directory` branch retains the explicit path. It does not execute a live `CreateProcessW` disappearance race, so this is not Green runtime coverage. |
