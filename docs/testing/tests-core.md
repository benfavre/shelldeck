...
| SDTEST-1755 | `workspace_review_tests.rs::sdtest_1755_attention_read_state_replays_and_duplicate_coordinates_fail_closed` | SDUC-493 | Yellow | Core local-read state survives an unchanged authoritative replay; attention resolves only through the retained state keyed by local workspace, so a foreign Browser surface sharing checkout/pane coordinates cannot substitute, and duplicate session coordinates fail closed. Retained GPUI entity/deep-link integration remains SDTEST-1758. |
