...
| ID | Location | SDUC | Status | Notes |
|---|---|---|---|---|
| SDTEST-1729 | `workspace_catalog.rs::catalog_v1_fixture_migrates_and_v2_cas_refuses_a_stale_writer` | SDUC-488 | Green | A pinned schema-v1 fixture migrates native local and POSIX SSH roots to schema v2, then two copies with the same persisted revision prove compare-and-swap rejects the stale writer. Future schemas and non-portable paths are separately refused. |
| SDTEST-1730 | `workspace_catalog.rs::session_binding_requires_the_exact_authoritative_workspace_mapping` | SDUC-488, SDUC-489 | Green | A local UUID record cannot bind an orchestration session before its explicit Platform v2 project/checkout/workspace mapping is exact, and the session must repeat that exact authoritative workspace ID. The external issue remains a separate record. |
| SDTEST-1731 | `workspace_navigation.rs::navigation_rejects_cross_workspace_terminal_reuse_and_stale_card_observations` | SDUC-490 | Green | A stable terminal binding cannot be owned by two local workspaces, a late card observation cannot overwrite newer facts, A→B→A retains the exact snapshot, and catalog archive clears active visibility. This is reducer evidence only, not a claim that a GPUI entity stayed alive. |
...
| SDTEST-1733 | `workspace_navigation.rs::creation_requires_strict_progress_retry_and_catalog_revision_fences` | SDUC-491 | Green | Completion from Queued is refused, a catalog revision change fences progress, only an exact typed revision conflict enables retry, and Start cannot bypass an existing job. |
...
| SDTEST-1735 | `util.rs::concurrent_atomic_writers_use_distinct_temporary_files` | SDUC-488 | Green | Two same-process writers start together, both complete through distinct `create_new` temporary files, the destination is one complete payload, and no temporary file remains. Catalog CAS supplies the higher-level lost-update fence. |
