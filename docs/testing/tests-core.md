...
| SDTEST-1735 | `util.rs::concurrent_atomic_writers_use_distinct_temporary_files` | SDUC-488 | Green | Two same-process writers start together, both complete through distinct `create_new` temporary files, the destination is one complete payload, and no temporary file remains. Unix uses same-filesystem rename; Windows uses `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)`. Catalog locking/CAS supplies the higher-level lost-update fence. |
