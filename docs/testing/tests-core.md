...
| SDTEST-1675 | `agent_runtime.rs::sdtest_1675_provider_streams_normalize_visible_text_and_errors` | SDUC-475 | Green | Claude, Codex, and Jcode JSONL fixtures normalize into provider-neutral text/error events; only Claude's `system/init` becomes Ready, while later hook/status control records stay out of the visible trace. |
