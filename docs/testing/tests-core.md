...
| SDTEST-1822 | `config/platform_attention.rs::sdtest_1822_activation_re_resolves_exact_current_catalogues_and_refuses_ambiguity` | SDUC-493 | Green | A captured authoritative item is resolved again against the current exact workspace mapping, full authority-qualified session directory, and retained provider panes. Exact Fleet and unique pane destinations succeed; missing/duplicate sessions, duplicate panes, and stale revisions refuse without first-match fallback. |
