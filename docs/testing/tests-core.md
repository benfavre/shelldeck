...
| SDTEST-1752 | `workspace_review_tests.rs::sdtest_1752_drafts_use_workspace_paths_monotone_cas_and_expected_identity` | SDUC-492 | Yellow | The core uses a workspace-keyed private-root path, expected-workspace admission, monotone disk revision and stale-writer CAS. Linked components/final files are refused and one no-follow descriptor is read to maximum-plus-one; the pre-anchor schema is explicitly refused because section/hunk identity cannot be reconstructed. A release-runner multiprocess/Windows-native filesystem exercise remains required. |
| SDTEST-1753 | `workspace_review_tests.rs::sdtest_1753_only_registered_unexpired_previews_can_submit` | SDUC-492 | Green | Provider authority cannot prepare repository work; a modified/deserialized preview, expired grant, revoked/superseded current repository grant, cross-workspace provider evidence, foreign surface, and duplicate comment IDs cannot cross durable submit admission. Comment anchors include exact section and unique hunk identity. Grant minting remains crate-private. |
| SDTEST-1754 | `workspace_review_tests.rs::sdtest_1754_dispatched_ledger_recovers_only_by_original_receipt` | SDUC-492 | Yellow | Drop/reload turns a durably dispatched operation into reconciliation-only work with the original key and exact receipt fences. Prepared, completed, and refused records remain enumerable after operation-ID loss; only explicit abandonment plus terminal acknowledgement releases their bounded slots. Forced process termination plus the real receipt adapter remains SDTEST-1758. |
| SDTEST-1755 | `workspace_review_tests.rs::sdtest_1755_attention_read_state_replays_and_duplicate_coordinates_fail_closed` | SDUC-493 | Yellow | Core local-read state survives an unchanged authoritative replay; catalog-invalid foreign surfaces and duplicate pane/session coordinates fail closed. Retained GPUI entity/deep-link integration remains SDTEST-1758. |
...
| SDTEST-1760 | `workspace_review_tests.rs::sdtest_1760_comment_batches_require_exact_workspace_surface_and_unique_anchors` | SDUC-492 | Green | Comment preparation/submission rejects matching-ID/actor/revision evidence when review, provider session, grant, or catalog-validated surface crosses workspace authority; duplicate comment IDs and section/hunk aliases fail closed. |
