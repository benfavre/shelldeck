...
| SDTEST-1848 | `config/platform_review/worktree.rs::sdtest_1848_staging_stays_withheld_for_every_capability_load` | SDUC-495 | Green | A snapshot proposal carrying a `git` authority is an observation. `advertised_staging_capability` is false for every capability load, so the projection reports `NoServerCapability` before the custody fence and exposes staging only as a per-file proposal observation; an unresolved conflict marks its own proposal inadmissible. |
