...
|---|---|---|---|---|---|---|
| REL-001 | NEXT-001 | Monique / activation | Un runtime désactivé ne doit jamais créer sa boucle, même avec des identifiants valides. | P0 | Terminé | SDTEST-272 couvre les quatre cas activation/identifiants ; la garde par itération reste en place. |
...
| REL-005 | — | Client de plateforme | L'ancien exécuteur local créait une seconde autorité d'exécution. | P1 | Terminé | L'exécuteur, les types HTTP dupliqués et la configuration runtime sont absents ; le cockpit utilise le SDK partagé pour les ressources, sessions et baux de contrôle. |
...
- **2026-08-18 — REL-007 terminé, sans défaut trouvé.** L'audit cherchait une
  ...
  Monique — `effective_monique_config` court-circuite sur `!signed_in()`. Le risque
...
- **2026-08-18 — Un implicite rendu explicite.** « Déconnecté ⇒ aucun polling »
  ...
  `has_monique` et `can_access_mode` exigent un compte). C'était vrai mais nulle
...
- **2026-08-18 — REL-006, deuxième lot : propriété d'un timeout.** L'inventaire
  annonçait « faux exécuteur / horloge » pour SDTEST-1366. Inutile : ce qui
  compte n'est pas *quand* le timer se déclenche mais *ce qu'il décide* à ce
  moment-là. La décision est extraite dans `ai_timeout_outcome`, appelée par les
  deux timers. Le garde existait déjà et était correct — mais il tenait en deux
  lignes qu'un refactor supprime sans y penser, et les supprimer signifie qu'un
  chronomètre appartenant à une exécution précédente tue une exécution plus
  récente. La mutation de contrôle le démontre.
- **2026-08-18 — REL-006 : un vrai défaut, pas un verrouillage.** Les trois lots
  précédents n'avaient trouvé que des contrats déjà corrects. Celui-ci non : le
  nommage IA d'un script ou d'un tunnel posait comme cible la **chaîne littérale**
  `"script-form"` / `"tunnel-form"`, qui ne désigne aucune entité. Comme une tâche
  de nommage peut être mise en attente puis reprise depuis le centre de tâches
  alors qu'un autre formulaire est ouvert, le nom accepté atterrissait sur ce
  dernier. La cible porte désormais l'identifiant d'entité GPUI du formulaire, et
