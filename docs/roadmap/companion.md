...
| 8 | AI Dock Companion | 🚧 phases A/B/C livrées : `Workspace`, vues/pollers, parsing SSH, store et Cloud Sync sont différés, avec `AiCompanionController` autonome et `CompanionRuntime` propriétaire du tray/raccourcis/fenêtres auxiliaires. Le benchmark Linux debug mesure −28 % sur le démarrage caché et un RSS inchangé. Phase D fonctionne sur Windows/macOS/Linux X11 et via le portail XDG sous Wayland ; Settings capture, réinitialise et applique immédiatement les combinaisons, avec conflits et résultats natifs visibles. Reste la documentation précise des permissions macOS. Les finitions sont détaillées dans [`ai-dock-companion.md`](ai-dock-companion.md). |
