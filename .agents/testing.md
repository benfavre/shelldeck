...
| **Contract mock** | same file, spawn `std::net::TcpListener` in a thread | ✅ | Every Manage API / Monique / Bext / Cloud client. The mock asserts *route*, *headers* (Bearer / Basic / `X-Bext-App-Id`), and *body shape*; the fixture pins the *response shape* the parser must handle. |
...
3. **Wire mocks the same way we already do.** For a new Manage-style
   ...
   `config/cloud_sync.rs`, `manage_support.rs`, `monique_fleet.rs`,
...
4. **For subprocess-touching code, add a fake trait.** See
   `JobExecutor` / `ClaudeExecutor` in `config/monique_fleet.rs` for the
