...
`shelldeck-core::config::monique_fleet` makes ShellDeck a runtime for the tenant/site-aware Monique fleet (`GET|POST {base}/api/manage/shelldeck/fleet`, Bearer). Types `MoniqueInstance`/`MoniqueJob`/`FleetSnapshot` (⚠️ **snake_case** matching the Manage fleet API; timestamps `created_at`/`updated_at`/`last_seen_at` come back as **ISO-8601 strings** → `de_flex_millis`). Client fns `get_fleet`/`register`/`heartbeat`/`claim`/`update_job`/`dispatch`. Execution goes through the `JobExecutor` trait — `ClaudeExecutor` mirrors the bot's `claude.ts` exactly (`claude -p --output-format stream-json --verbose --permission-mode acceptEdits [--model]`, prompt on stdin, drops `ANTHROPIC_API_KEY`, keeps `CLAUDE_CODE_OAUTH_TOKEN`, cwd=workdir, 30-min SIGKILL, parses the final `result` event) — so `runtime_tick`/`execute_job` are unit-tested with a **fake** executor (`claude` never runs in tests). `[monique_runtime]` config (`enabled` default **false**, `instance_id` persisted after first register, `workdir`/`name`). **SAFETY**: `Workspace::sync_runtime_loop` (register→heartbeat→claim→execute, 15s, concurrency 1 via `runtime_busy`) only auto-executes when `enabled && instance.autonomy=="auto"`; `"confirm"` (the register default) leaves the claimed job in `runtime_awaiting` for an explicit Exécuter/Rejeter in `FleetView` (`fleet_view.rs`, `ActiveView::Fleet`). Actions `OpenFleet`/`ToggleMoniqueRuntime`. ⚠️ verification = `monique_fleet.rs` mock TcpListener + fake executor (register/claim/job flow, auto vs confirm gating, 401) + the live shapes confirmed by curling GET fleet + a throwaway register/heartbeat/claim (no `claude` executed). ⚠️ a throwaway instance `4365eee9-391f-4eae-a11f-494374e1eda6` (tenant fghfg) was left parked offline — safe to wipe.
