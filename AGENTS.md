...
### Monique fleet client

`shelldeck-core::config::monique_fleet` is the typed client for the tenant/site-aware fleet (`GET|POST {base}/api/manage/shelldeck/fleet`, Bearer). Types `MoniqueInstance`/`MoniqueJob`/`FleetSnapshot` use the API's snake_case and flexible timestamps. ShellDeck reads scoped state and sends typed server-side actions only; it never registers as an executor, claims work, or launches a provider subprocess. Old `[monique_runtime]` fields remain parseable for config compatibility, but `Workspace::sync_runtime_loop` always clears enablement and starts no task. AI Operations owns execution, idempotency, policy, and receipts.
