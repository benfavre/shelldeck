...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .mode-list span{display:flex;flex-direction:column;gap:5px;padding:12px;border-radius:12px;color:#77808c;font-size:12px;transition:transform .2s ease,background .2s ease}.mode-list span:hover{transform:translateY(-3px);background:#f3f7fd}
    ...
    @media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.button,.site-header,.nav-cta,.nav-cta svg,.text-link span,.hero-stage,.mode-list span,.layer,.proof-strip span,.tool-grid article,.flow-step,.download-card{transition:none!important}.hero-copy>*,.hero-stage,.bubble,.journey-line path,.layer-detail.is-updating strong,.layer-detail.is-updating p{animation:none!important}.motion-ready [data-reveal]{opacity:1!important;transform:none!important;transition:none!important}.hero-stage{background-image:url('/campaign/roles-v1/hero-desktop-poster.webp');transform:none!important}.hero-motion{display:none!important}}
  ...
  <script>
    (function(){
      ...
      if('IntersectionObserver' in window&&!reduced.matches){
        var groups=['.mode-list span','.ecosystem-head','.layer-grid .layer','.proof-strip span','.section-head','.scene','.ai-copy','.ai-window','.dev-heading','.tool-grid article','.inklura-copy','.flow-step','.download-head','.download-card','.install-box'];var reveals=[];
        try{
