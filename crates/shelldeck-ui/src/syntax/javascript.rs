...
fn keywords() -> &'static HashSet<&'static str> {
    JS_KEYWORDS.get_or_init(|| {
        [
            ...
            "public", "abstract", "type", "declare", "namespace", "module",
            // Bun/TS specifics
            "as", "satisfies", "readonly", "keyof", "infer",
        ]
        .into_iter()
        .collect()
    })
...
fn builtins() -> &'static HashSet<&'static str> {
    JS_BUILTINS.get_or_init(|| {
        [
            ...
            "Proxy", "Reflect", "Error", "RegExp", "parseInt", "parseFloat",
            "isNaN", "isFinite", "undefined", "null", "true", "false", "NaN",
            "Infinity", "globalThis", "fetch", "Response", "Request", "URL",
            // Bun specifics
            "Bun",
        ]
        .into_iter()
...
pub fn tokenize(source: &str) -> Vec<Token> {
    ...
    while i < len {
        ...
        if b.is_ascii_digit() || (b == b'.' && i + 1 < len && bytes[i + 1].is_ascii_digit()) {
            ...
            if b == b'0' && i + 1 < len && matches!(bytes[i + 1], b'x' | b'X' | b'b' | b'B' | b'o' | b'O') {
                i += 2;
        ...
        // Arrow operator
        if b == b'=' && i + 1 < len && bytes[i + 1] == b'>' {
            tokens.push(Token { range: i..i + 2, kind: TokenKind::Operator });
            i += 2;
            continue;
        }
