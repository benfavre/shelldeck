...
fn main() {
    let base_url = std::env::var("BEXT_CLOUD_URL").unwrap_or_else(|_| "https://cloud.bext.dev".to_string());
    let token = std::env::var("BEXT_CLOUD_TOKEN").unwrap_or_default();
    if !token.is_empty() {
        let cfg = BextCloudConfig { base_url, token, email: String::new(), name: String::new() };
        match bext_cloud::whoami(&cfg) {
            Ok(u) => println!("whoami: {} <{}> super_admin={}", u.name, u.email, u.is_super_admin),
            Err(e) => { eprintln!("whoami failed: {}", e); std::process::exit(1); }
        }
        ...
        match bext_cloud::dashboard(&cfg) {
            Ok(d) => println!("dashboard stats: projects={} deploys={} domains={} targets={}", d.stats.projects, d.stats.deploys, d.stats.domains, d.stats.targets),
            Err(e) => eprintln!("dashboard failed: {}", e),
        }
        match bext_cloud::list_instances(&cfg) {
            Ok(i) => { println!("instances: {}", i.total); for x in &i.instances { println!("  - {} [{}] {} / {}", x.name, x.status, x.health, x.url); } }
    ...
    if let Ok(url) = std::env::var("BEXT_INSTANCE_URL") {
        let inst = BextInstance::new(url, std::env::var("BEXT_INSTANCE_APP_ID").unwrap_or_else(|_| "cloud-bext".into()));
        match bext_instance::list_sites(&inst) {
            Ok(s) => { println!("instance sites: {}", s.sites.len()); for x in s.sites.iter().take(5) { println!("  - {} [{}] {}", x.slug, x.kind, x.primary_domain); } }
