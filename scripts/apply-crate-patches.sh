...
apply_portable_pty() {
    ...
    dir="$(find "$REGISTRY" -maxdepth 2 -type d -name 'portable-pty-0.9.0' 2>/dev/null | head -1)"
    if [[ -z "$dir" ]]; then
        echo "apply-crate-patches: portable-pty-0.9.0 not in registry after cargo fetch" >&2
        exit 1
    fi
